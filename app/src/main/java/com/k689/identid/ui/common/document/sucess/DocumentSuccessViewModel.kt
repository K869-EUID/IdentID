...
sealed class Event : ViewEvent {
    ...
    data class ExpandOrCollapseSuccessDocumentItem(
        val itemId: String,
    ) : Event()
}
...
abstract class DocumentSuccessViewModel : MviViewModel<Event, State, Effect>() {
    ...
    override fun handleEvents(event: Event) {
        when (event) {
            ...
            is Event.ExpandOrCollapseSuccessDocumentItem -> {
                expandOrCollapseSuccessDocumentItem(id = event.itemId)
            }
    ...
    private fun expandOrCollapseSuccessDocumentItem(id: String) {
        ...
        val updatedItems =
            currentItems.map { successDocument ->
                val newHeader =
                    if (successDocument.header.itemId == id) {
                        val newIsExpanded = !successDocument.isExpanded
                        val newCollapsed =
                            successDocument.header.copy(
                                trailingContentData =
