...
package com.k689.identid.util.business
...
fun LocalDateTime.isJustNow(): Boolean = ChronoUnit.MINUTES.between(this, LocalDateTime.now()) == 0L

fun LocalDateTime.isWithinLastHour(): Boolean = ChronoUnit.MINUTES.between(this, LocalDateTime.now()) < 60

fun LocalDateTime.minutesToNow(): Long = ChronoUnit.MINUTES.between(this, LocalDateTime.now())

fun LocalDateTime.isToday(): Boolean = this.toLocalDate() == LocalDateTime.now().toLocalDate()
...
fun LocalDateTime.startOfDay(): LocalDateTime = this.withHour(0).withMinute(0).withSecond(0)

fun LocalDateTime.endOfDay(): LocalDateTime = this.withHour(23).withMinute(59).withSecond(59)

fun LocalDateTime.startOfWeek(): LocalDateTime = this.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY)).startOfDay()

fun LocalDateTime.endOfWeek(): LocalDateTime = this.with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY)).endOfDay()

fun LocalDateTime.startOfMonth(): LocalDateTime = this.with(TemporalAdjusters.firstDayOfMonth()).startOfDay()

fun LocalDateTime.endOfMonth(): LocalDateTime = this.with(TemporalAdjusters.lastDayOfMonth()).endOfDay()
...
fun LocalDate?.toDisplayedDate(): String = this?.let { convertLocalDateToFormattedString(it) }.orEmpty()

fun Instant.toLocalDateTime(zoneId: ZoneId = ZoneId.systemDefault()): LocalDateTime = LocalDateTime.ofInstant(this, zoneId)

fun Instant.toLocalDate(zoneId: ZoneId = ZoneId.systemDefault()): LocalDate = this.atZone(zoneId).toLocalDate()

fun utcMillisToLocalDate(
    utcMillis: Long,
    zoneId: ZoneId = ZoneId.systemDefault(),
): LocalDate = Instant.ofEpochMilli(utcMillis).atZone(zoneId).toLocalDate()

fun localDateToUtcMillis(localDate: LocalDate): Long = localDateToMillis(localDate = localDate, zoneId = ZoneOffset.UTC)

fun localDateToMillis(
    localDate: LocalDate,
    zoneId: ZoneId = ZoneId.systemDefault(),
): Long = localDate.atStartOfDay(zoneId).toInstant().toEpochMilli()

fun LocalDate.toLocalDateTime(time: LocalTime = LocalTime.MIDNIGHT): LocalDateTime = this.atTime(time)
